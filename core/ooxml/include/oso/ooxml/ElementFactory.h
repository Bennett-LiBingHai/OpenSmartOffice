...
public:
    ...
    /// 注册一个元素类型的创建函数
    void registerCreator(const std::string& namespaceUri,
                         const std::string& localName,
                         CreatorFn creator);
    ...
    /// 检查某个元素类型是否已注册
    bool isRegistered(const std::string& namespaceUri,
                      const std::string& localName) const;
...
private:
    ...
    static std::string makeKey(const std::string& namespaceUri,
                               const std::string& localName);
