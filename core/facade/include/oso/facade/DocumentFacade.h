...
public:
    ...
    static Result<std::unique_ptr<DomNode>> openDocument(const std::string& path);

    /// 保存 DOM 文档到 OOXML 文件
    /// @param doc 文档根节点
    /// @param path 输出文件路径
    /// @return 成功返回 OK
    static Result<void> saveDocument(const DomNode& doc, const std::string& path);
...
private:
    ...
    /// 加载 Word 文档的各个部件
    static Result<std::unique_ptr<DomNode>> loadWordDocument(class IZipArchive& archive);

    /// 保存 Word 文档
    static Result<void> saveWordDocument(const DomNode& doc, const std::string& path);
};
