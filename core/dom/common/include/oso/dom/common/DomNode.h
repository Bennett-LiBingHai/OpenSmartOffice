...
protected:
    ...
    /// 写出当前元素的结束标签
    static void serializeEndElement(IOoxmlWriter& writer);
...
public:
    explicit DomDocument(std::string documentType = "")
        : DomNode(DomNodeType::Document), m_documentType(std::move(documentType)) {}
