...
int writeCallback(void* context, const char* buffer, int len) {
    ...
    auto result = stream->write(reinterpret_cast<const uint8_t*>(buffer),
...
struct Libxml2Writer::Impl {
    ...
    std::string findPrefix(const std::string& nsUri) const {
        if (nsUri.empty()) return {};
        const auto& map = OoxmlNamespaces::prefixMap();
        for (const auto& [prefix, uri] : map) {
            if (uri == nsUri) return std::string(prefix);
        }
        return {};
    }

    /// 写出 xmlns:prefix="nsUri"，仅在首次遇到时声明。
    Result<void> declareNamespace(const std::string& nsUri,
                                   const std::string& prefix) {
        if (nsUri.empty() || prefix.empty()) return Result<void>::ok();
        if (declaredNs.count(nsUri)) return Result<void>::ok();

        std::string attrName = "xmlns:" + prefix;
        int rc = xmlTextWriterWriteAttribute(
            writer, BAD_CAST attrName.c_str(), BAD_CAST nsUri.c_str());
        if (rc < 0) {
...
void Libxml2Writer::setOutput(IStream& stream) {
    ...
    m_impl->outBuf = xmlOutputBufferCreateIO(
        writeCallback,
        closeCallback,
        &stream,
...
Result<void> Libxml2Writer::writeStartDocument(bool standalone) {
    ...
    int rc = xmlTextWriterStartDocument(
        m_impl->writer,
        nullptr,  // version — nullptr = "1.0"
        "UTF-8",
        standalone ? "yes" : nullptr);
...
Result<void> Libxml2Writer::writeStartElement(
    ...
    int rc = xmlTextWriterStartElement(m_impl->writer,
                                       BAD_CAST qName.c_str());
    if (rc < 0) {
        m_impl->hasError = true;
        return Result<void>::err(ErrorCode::OOXML_XmlParseError,
    ...
    if (declareResult.isErr()) return declareResult;
...
Result<void> Libxml2Writer::writeAttribute(
    const std::string& namespaceUri,
    const std::string& localName,
...
Result<void> Libxml2Writer::declareNamespace(const std::string& namespaceUri) {
    ...
    if (namespaceUri.empty()) return Result<void>::ok();
...
Result<void> Libxml2Writer::writeText(const std::string& text) {
    ...
    // 会自动转义
    int rc = xmlTextWriterWriteString(
        m_impl->writer,
        BAD_CAST text.c_str());
