...
const std::unordered_map<std::string_view, std::string_view> ContentTypeRegistry::kPartMime = {
    ...
    {"word/comments",       "application/vnd.openxmlformats-officedocument.wordprocessingml.comments+xml"},
    {"word/settings.xml",   "application/vnd.openxmlformats-officedocument.wordprocessingml.settings+xml"},
    {"word/endnotes",       "application/vnd.openxmlformats-officedocument.wordprocessingml.endnotes+xml"},
    {"word/fontTable.xml",  "application/vnd.openxmlformats-officedocument.wordprocessingml.fontTable+xml"},
    {"word/footer",         "application/vnd.openxmlformats-officedocument.wordprocessingml.footer+xml"},
    {"word/footnotes",      "application/vnd.openxmlformats-officedocument.wordprocessingml.footnotes+xml"},
    ...
    {"xl/worksheets/sheet",         "application/vnd.openxmlformats-officedocument.spreadsheetml.worksheet+xml"},
    ...
    {"xl/chartsheets/sheet",        "application/vnd.openxmlformats-officedocument.spreadsheetml.chartsheet+xml"},
    ...
    {"xl/pivotTables/pivotTable",   "application/vnd.openxmlformats-officedocument.spreadsheetml.pivotTable+xml"},
    {"xl/pivotCache/pivotCacheDefinition", "application/vnd.openxmlformats-officedocument.spreadsheetml.pivotCacheDefinition+xml"},
    {"xl/pivotCache/pivotCacheRecords",    "application/vnd.openxmlformats-officedocument.spreadsheetml.pivotCacheRecords+xml"},
    {"xl/tables/table",             "application/vnd.openxmlformats-officedocument.spreadsheetml.table+xml"},
    {"xl/queryTables/queryTable",   "application/vnd.openxmlformats-officedocument.spreadsheetml.queryTable+xml"},
    {"xl/externalLinks/externalLink","application/vnd.openxmlformats-officedocument.spreadsheetml.externalLink+xml"},
    {"xl/metadata.xml",             "application/vnd.openxmlformats-officedocument.spreadsheetml.sheetMetadata+xml"},
    {"xl/revisions/revisionHeaders", "application/vnd.openxmlformats-officedocument.spreadsheetml.revisionHeaders+xml"},
    {"xl/revisions/revisionLog",    "application/vnd.openxmlformats-officedocument.spreadsheetml.revisionLog+xml"},
    {"xl/revisions/userNames",      "application/vnd.openxmlformats-officedocument.spreadsheetml.userNames+xml"},
    {"xl/tables/tableSingleCells",  "application/vnd.openxmlformats-officedocument.spreadsheetml.tableSingleCells+xml"},
    ...
    {"diagrams/colors",         "application/vnd.openxmlformats-officedocument.drawingml.diagramColors+xml"},
    {"diagrams/data",           "application/vnd.openxmlformats-officedocument.drawingml.diagramData+xml"},
    {"diagrams/layout",         "application/vnd.openxmlformats-officedocument.drawingml.diagramLayout+xml"},
    {"diagrams/quickStyle",     "application/vnd.openxmlformats-officedocument.drawingml.diagramStyle+xml"},
...
std::string_view ContentTypeRegistry::lookupOverride(const std::string& partName) {
    ...
    for (const auto& [name, mime] : kPartMime) {
        if (partName == name) return mime;
    }
    // 再前缀匹配（用于编号文件：sheet1.xml / sheet2.xml / slide3.xml 等）
    for (const auto& [name, mime] : kPartMime) {
        std::size_t len = name.size();
        if (partName.size() > len &&
            partName.compare(0, len, name) == 0) {
            return mime;
        }
    }
    return "";
...
std::vector<uint8_t> ContentTypeRegistry::generate(const std::vector<std::string>& partNames) {
    ...
    for (const auto& [ext, mime] : extraDefaults) {
        xml << "  <Default Extension=\"" << ext
            << "\" ContentType=\"" << mime << "\"/>\n";
    }

    for (const auto& [path, mime] : overrides) {
        xml << "  <Override PartName=\"/" << path
            << "\" ContentType=\"" << mime << "\"/>\n";
    }
...
Result<ContentTypeRegistry> ContentTypeRegistry::parse(const std::vector<uint8_t>& xmlData) {
    if (xmlData.empty()) {
        return Result<ContentTypeRegistry>::err(ErrorCode::OOXML_XmlParseError, "Empty [Content_Types].xml data");
    }
    ...
    if (!doc) {
        return Result<ContentTypeRegistry>::err(ErrorCode::OOXML_XmlParseError, "Failed to parse [Content_Types].xml");
    }
    ...
    for (xmlNodePtr child = root->children; child; child = child->next) {
        ...
        xmlChar* partName = xmlGetProp(child, reinterpret_cast<const xmlChar*>(isOverride ? "PartName" : "Extension"));
