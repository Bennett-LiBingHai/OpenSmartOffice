...
Result<void> LibzipZipArchive::open(const std::string& path) {
    if (m_archive != nullptr) {
        return Result<void>::err(ErrorCode::OOXMLZipCorrupted, "Archive already open");
    }
    ...
    if (m_archive == nullptr) {
        switch (error) {
            case ZIP_ER_NOENT:
                return Result<void>::err(ErrorCode::IOFileNotFound, "File not found: " + path);
            case ZIP_ER_NOZIP:
                return Result<void>::err(ErrorCode::OOXMLZipCorrupted,
                                         "Not a valid zip file: " + path);
            case ZIP_ER_OPEN:
                return Result<void>::err(ErrorCode::IOAccessDenied, "Cannot open file: " + path);
            case ZIP_ER_READ:
                return Result<void>::err(ErrorCode::IOReadError, "Read error: " + path);
            default:
                return Result<void>::err(
                    ErrorCode::OOXMLZipCorrupted,
...
Result<void> LibzipZipArchive::create(const std::string& path) {
    ...
    if (m_archive == nullptr) {
        switch (error) {
            case ZIP_ER_OPEN:
                return Result<void>::err(ErrorCode::IOAccessDenied, "Cannot create file: " + path);
            case ZIP_ER_WRITE:
                return Result<void>::err(ErrorCode::IOWriteError, "Write error creating: " + path);
            default:
                return Result<void>::err(
                    ErrorCode::OOXMLZipWriteError,
...
Result<void> LibzipZipArchive::writeEntry(const std::string& name,
    ...
    if (m_archive == nullptr) {
        return Result<void>::err(ErrorCode::OOXMLZipWriteError, "Archive not created");
    }

    // 拷贝数据到堆上，libzip 用 freep=1 接管所有权，在 source 释放时调用 free()
    auto* buf = static_cast<uint8_t*>(std::malloc(data.size()));
    if (buf == nullptr) {
        return Result<void>::err(ErrorCode::OOXMLZipWriteError,
...
Result<std::vector<IZipArchive::Entry>> LibzipZipArchive::entries() const {
    if (m_archive == nullptr) {
        return Result<std::vector<Entry>>::err(ErrorCode::OOXMLZipCorrupted, "Archive not open");
    }
...
Result<std::vector<uint8_t>> LibzipZipArchive::readEntry(const std::string& name) {
    if (m_archive == nullptr) {
        return Result<std::vector<uint8_t>>::err(ErrorCode::OOXMLZipCorrupted, "Archive not open");
    }

    zip_int64_t idx = zip_name_locate(m_archive, name.c_str(), 0);
    if (idx < 0) {
        return Result<std::vector<uint8_t>>::err(ErrorCode::OOXMLZipEntryMissing,
    ...
    if (bytesRead < 0 || static_cast<zip_uint64_t>(bytesRead) != stat.size) {
        return Result<std::vector<uint8_t>>::err(ErrorCode::IOReadError,
...
Result<void> LibzipZipArchive::close() {
    ...
    if (zip_close(m_archive) != 0) {
        ...
        m_archive = nullptr;
        return Result<void>::err(ErrorCode::OOXMLZipCorrupted, "Failed to close archive: " + err);
    }
...
bool LibzipZipArchive::isOpen() const { return m_archive != nullptr; }
