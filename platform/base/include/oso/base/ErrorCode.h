...
public:
    ...
    Value value() const { return m_value; }           ///< 获取错误码枚举值
    bool isOk() const { return m_value == Ok; }       ///< 是否成功
    bool isErr() const { return m_value != Ok; }      ///< 是否失败

    std::string toString() const;                     ///< 转为可读字符串
    uint32_t raw() const { return static_cast<uint32_t>(m_value); }  ///< 获取原始整数值
