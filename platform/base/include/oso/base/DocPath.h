...
public:
    ...
    static PartPath fromString(std::string_view sv) { return PartPath(std::string(sv)); }
...
public:
    ...
    [[nodiscard]] const char* cStr() const { return m_id.c_str(); }
...
public:
    ...
    explicit ExternalReference(std::string uri) : m_uri(std::move(uri)) { detectType(); }
    ...
    [[nodiscard]] const char* cStr() const { return m_uri.c_str(); }
    [[nodiscard]] Type type() const { return m_type; }
    [[nodiscard]] bool empty() const { return m_uri.empty(); }

    // 是否包含 URI scheme（如 http:, file:, mailto:）
    [[nodiscard]] bool hasScheme() const { return hasUriScheme(m_uri); }

    // 是否为相对引用（非绝对、无 scheme）
    [[nodiscard]] bool isRelative() const { return !m_uri.empty() && !isAbsolute(); }

    // 是否为绝对引用（有 scheme 或以 "//" 开头）
    [[nodiscard]] bool isAbsolute() const { return hasUriScheme(m_uri) || m_uri.starts_with("//"); }
...
private:
    ...
    static bool isWindowsDrivePrefix(std::string_view s) {
        return s.size() >= 2 && std::isalpha(static_cast<unsigned char>(s[0])) != 0 && s[1] == ':';
    }
