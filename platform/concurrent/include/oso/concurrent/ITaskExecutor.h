...
public:
    ...
    virtual void submit(std::function<void()> task, int priority) = 0;

    // 默认0优先级
    void submit(std::function<void()> task) { submit(std::move(task), 0); }

    /// 等待所有已提交任务完成。
    /// @param timeout  超时时间，小于等于 0 表示无限等待，单位毫秒
    /// @return  成功返回 Ok，超时返回 Concurrent_Timeout
    virtual Result<void> waitAll(int64_t timeout) = 0;
