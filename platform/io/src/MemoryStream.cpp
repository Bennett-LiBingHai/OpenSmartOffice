...
Result<size_t> MemoryStream::read(uint8_t* buffer, size_t maxLen) {
    if (!m_open)
        return Result<size_t>::err(ErrorCode::IO_ReadError, "Stream closed");
...
Result<std::vector<uint8_t>> MemoryStream::readAll() {
    if (!m_open)
        return Result<std::vector<uint8_t>>::err(ErrorCode::IO_ReadError, "Stream closed");
...
Result<void> MemoryStream::write(const uint8_t* data, size_t len) {
    if (!m_open)
        return Result<void>::err(ErrorCode::IO_WriteError, "Stream closed");
