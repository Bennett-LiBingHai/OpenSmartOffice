...
class IFileSystem {
   public:
    ...
    /// 列出目录内容
    virtual Result<std::vector<std::string>> listDirectory(const std::string& path) const = 0;
    /// 创建目录（包括必要的父目录）
...
class NativeFileSystem : public IFileSystem {
   public:
    ...
    Result<std::vector<std::string>> listDirectory(const std::string& path) const override;
